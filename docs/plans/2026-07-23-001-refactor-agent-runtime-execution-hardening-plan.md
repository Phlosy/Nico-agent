...
Migrations `0027` through `0034` form one linear revision chain, never rewrite `0026`, and default to forward-fix for durable Runtime evidence rather than destructive down migration. Migration `0028` stores the Agent-scoped chat defaults introduced after Goal A; the remaining Runtime Hardening schema Goals continue from `0029`.
...
- **Test scenarios:**
  ...
  3. A representative `0026` dataset upgrades through one `0034` head with no orphans, illegal states or missing backfills.
