...
| 四类 Memory 与作用域 | 四类型、tenant/project/agent/team scope 与生命周期已冻结 | 三类可验证 scope、候选生成、不可变修订链、原子发布/索引、失效/到期/tombstone 与检索已实现；team 在 Goal G 前失败关闭 | 状态、chunk/embed、反思、验证和生命周期规则已覆盖 | RLS、scope、working TTL、修订/supersede、失效/到期/删除及召回排除通过 | 未实现 | Goal F 计划与 Memory/Skill 边界已更新 | F5 Memory 生命周期已验证；API 待 F7 |
...
| Skill 与不可变 SkillVersion | 稳定身份、不可变版本、结构化内容与 deployment 已冻结 | 递增 Draft、八区差异、双闸门发布、稳定指针、Run 分桶 canary、推广/退役/弃用/禁用/回滚服务及数据库防线已实现 | 比较、分桶、方向和工具差异已覆盖 | 首发/修订/发布/并发灰度/解析/推广/同版恢复/历史回滚/禁用/RLS 与直接 SQL 绕过通过 | 未实现 | 状态机、领域与 Memory/Skill 边界已更新 | F6 服务已验证；API 待 F7 |
| Candidate、验证、审批、发布与回滚 | content-hash 绑定的 Candidate→Evaluation→Approval→发布/灰度/回滚已冻结 | 反思、确定性 validator、不可变 Evaluation、非自审 Approval、Memory 与 Skill 双闸门发布、灰度和回滚均已实现 | 候选、Hash/Schema/工具/来源验证、verdict、比较和分桶已覆盖 | 并发幂等、最新评价、审批全终态、原子 Memory 发布、Skill 工具漂移/推广/回滚、来源不可改写通过 | 未实现 | ADR-0005/0010 与阶段计划已完成 | F6 受控成长闭环已验证；API 待 F7 |
...
| PostgreSQL/pgvector/Redis/MinIO | 已决策 | Runtime/Tool、成长表、vector(384)/HNSW、FORCE RLS、评价/审批唯一索引、不可变来源、原子 Memory 发布与 Skill release guards 已落库 | 探针/状态/chunk/embed/反思/验证/分桶已完成 | 57 项真实依赖/控制面/Runtime/Tool/Growth/vector 测试通过 | API、Worker 与 Tool Gateway 使用真实 PostgreSQL 已通过；Goal F E2E 待 F8 | ADR、领域、Tool、Memory/Skill 文档已更新 | Goal F F6 Skill 生命周期已验证 |
...
| 全量单元、集成、故障与 E2E 测试 | 已规划 | Goal E 测试出口及 Goal F F2–F6 数据库/检索/候选/审核/Memory/Skill 生命周期测试已实现 | 后端 170 + 前端 7 通过 | 真实依赖 57 项通过 | Goal C 核心、Goal D Runtime、Goal E Tool/Sandbox E2E 通过；Goal F E2E 未实现 | `docs/testing.md` | Goal F F6 回归已通过 |
