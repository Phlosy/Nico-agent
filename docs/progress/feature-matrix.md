...
| 四类 Memory 与作用域 | 四类型、tenant/project/agent/team scope 与生命周期已冻结 | 三类可验证 scope、候选生成、不可变修订链、原子发布/索引、失效/到期/tombstone、检索及 REST API 已实现；team 在 Goal G 前失败关闭 | 状态、chunk/embed、反思、验证、生命周期和 OpenAPI 规则已覆盖 | RLS、scope、HTTP 双租户、working TTL、修订/supersede、发布/检索及召回排除通过 | F8 待实现 | Goal F 计划、API 与 Memory/Skill 边界已更新 | F7 Memory API 已验证 |
| pgvector 语义检索与来源追踪 | scope-first 检索、确定性 chunk/embed、完整来源快照已冻结 | terminal GrowthSource、确定性切片/384 维 embedding、HNSW cosine，以及有界脱敏 TrajectorySnapshot 与精确工具/Runtime 来源已实现 | normalization/chunk/embed、递归脱敏和嵌套 Hash 稳定性通过 | 真实 vector、RLS/scope/来源/失效/不可变及明文 Secret 排除通过 | 未实现 | ADR-0010 与检索/反思边界已完成 | F4 已验证；本地 embedding 为质量基线 |
| Skill 与不可变 SkillVersion | 稳定身份、不可变版本、结构化内容与 deployment 已冻结 | 递增 Draft、八区差异、双闸门发布、稳定指针、Run 分桶 canary、推广/退役/弃用/禁用/回滚服务、REST API 及数据库防线已实现 | 比较、分桶、方向、工具差异和 OpenAPI 已覆盖 | HTTP 首发/修订/发布/灰度/解析/推广/历史回滚/禁用/RLS 与直接 SQL 绕过通过 | F8 待实现 | 状态机、API、领域与 Memory/Skill 边界已更新 | F7 Skill API 已验证 |
| Candidate、验证、审批、发布与回滚 | content-hash 绑定的 Candidate→Evaluation→Approval→发布/灰度/回滚已冻结 | 反思、确定性 validator、不可变 Evaluation、非自审 Approval、Memory 与 Skill 双闸门发布、灰度、回滚及 HTTP 命令均已实现 | 候选、Hash/Schema/工具/来源验证、verdict、比较、分桶、敏感字段排除已覆盖 | HTTP 完整闭环、403/404/409/422、双租户、并发幂等、工具漂移与来源不可改写通过 | F8 待实现 | ADR-0005/0010、API 与阶段计划已完成 | F7 受控成长 API 已验证 |
...
| REST API 与 OpenAPI | 已设计 | 健康、控制面、Runtime/Tool 查询，以及 Goal F 候选、Memory、Skill/Version、Evaluation/Approval、deployment 全生命周期已实现 | 172 单测含路径/Header/成长响应敏感字段排除 | 58 项真实集成含完整成长 HTTP 与第二租户隔离 | Goal F E2E 待 F8 | `docs/api.md` | Goal F F7 已验证；认证/SSE/SDK 待后续 |
...
| 全量单元、集成、故障与 E2E 测试 | 已规划 | Goal E 测试出口及 Goal F F2–F7 数据库/检索/候选/审核/Memory/Skill 服务与 API 测试已实现 | 后端 172 + 前端 7 通过 | 真实依赖 58 项通过 | Goal C 核心、Goal D Runtime、Goal E Tool/Sandbox E2E 通过；Goal F E2E 待 F8 | `docs/testing.md` | Goal F F7 回归已通过 |
