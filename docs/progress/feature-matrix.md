...
| 四类 Memory 与作用域 | 四类型、tenant/project/agent/team scope 与生命周期已冻结 | Memory 版本、三类可验证 scope、状态/过期/tombstone、索引/检索及策略控制的候选生成已实现；team 在 Goal G 前失败关闭 | 状态、chunk/embed、反思结果和策略规则已覆盖 | RLS、scope 集合、Candidate 排除、失败 Run working TTL 与策略所有权通过 | 未实现 | Goal F 计划与 Memory/Skill 边界已更新 | F4 候选生成已验证；生命周期服务/API 待实现 |
...
| Candidate、验证、审批、发布与回滚 | content-hash 绑定的 Candidate→Evaluation→Approval→发布/灰度/回滚已冻结 | provider-neutral 反思、确定性基线、Run 锁幂等 Memory/Skill Candidate、Event/Audit 与数据库闸门已实现；验证/发布/回滚服务未实现 | 成功/失败候选、策略、Hash、脱敏与 DTO 约束已覆盖 | 并发幂等、来源闭合、候选不可召回、审计与直接发布拒绝通过 | 未实现 | ADR-0005/0010 与阶段计划已完成 | F4 候选阶段已验证；F5/F6 待实现 |
...
| PostgreSQL/pgvector/Redis/MinIO | 已决策 | Runtime/Tool、成长表、MemoryChunk vector(384)/HNSW、运行角色/FORCE RLS 与候选事务已落库 | 探针/状态/chunk/embed/反思已完成 | 44 项真实依赖/控制面/Runtime/Tool/Growth/vector 测试通过 | API、Worker 与 Tool Gateway 使用真实 PostgreSQL 已通过；Memory E2E 待 F8 | ADR、领域、Tool、Memory/Skill 文档已更新 | Goal F F4 候选范围已验证 |
...
| 全量单元、集成、故障与 E2E 测试 | 已规划 | Goal E 测试出口及 Goal F F2–F4 数据库/检索/候选测试已实现 | 后端 163 + 前端 7 通过 | 真实依赖 44 项通过 | Goal C 核心、Goal D Runtime、Goal E Tool/Sandbox E2E 通过；Goal F E2E 未实现 | `docs/testing.md` | Goal F F4 回归已通过 |
