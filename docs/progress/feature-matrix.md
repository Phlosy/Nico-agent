...
| RunStep、ModelCall 与 ToolCall 轨迹 | 已设计 | Runtime 事件映射 RunStep、Event 与轨迹已实现；ModelCall/ToolCall 待 Goal E | RunStep 与 Runtime event contract 已覆盖 | Step/Event/trajectory 原子持久化已通过 | Runtime Step 完整路径已通过 | Goal D 边界已记录 | Runtime 轨迹已验证；ModelCall/ToolCall 未实现 |
...
| HermesRuntimeProvider | 已设计 | 隔离 CLI Adapter、版本 fail-closed、历史 resume、进程组取消、脱敏导出已实现 | 假 CLI 7 项边界/本地 0.18.2 源码兼容已通过 | 无外部模型凭据，不声称真实推理 | 不适用：可选外部依赖未安装 | 进程与能力边界已完成 | Goal D Adapter 已验证；真实推理未执行 |
...
| REST API 与 OpenAPI | 已设计 | 健康、控制面、RuntimeSession/Trajectory API 已实现 | API/OpenAPI 单测已通过 | 5 项控制面场景及基础设施通过 | 核心 HTTP 与 Runtime Worker 生命周期已通过 | `docs/api.md` | Goal D 查询范围已验证；认证/SSE/SDK 待后续 |
...
| PostgreSQL/pgvector/Redis/MinIO | 已决策 | RuntimeSession、claimer 函数、运行角色/RLS 已加入现有 Compose/控制面 | 探针/状态规则已完成 | 18 项真实依赖/控制面/Runtime 测试通过 | API 与 Worker 使用真实 PostgreSQL 已通过 | ADR、领域与测试文档已完成 | Goal D 存储范围已验证 |
| Docker Compose 与一键脚本 | 已规划 | 7 服务拓扑、持久化 Worker 与 Goal D 验收脚本已实现 | Shell/Compose 静态校验通过 | 镜像、迁移、依赖健康已验证 | `verify-goal-d.sh` 出口已实现 | 开发/测试文档已完成 | Goal D 已验证 |
| 全量单元、集成、故障与 E2E 测试 | 已规划 | Goal D 测试出口已实现 | 后端 47 + 前端 7 通过 | 真实依赖 18 项通过 | Goal C 核心与 Goal D Runtime E2E 通过 | `docs/testing.md` | Goal D 已验证；Tool 故障待 Goal E |
