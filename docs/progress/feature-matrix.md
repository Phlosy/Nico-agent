...
| 四类 Memory 与作用域 | 四类型、tenant/project/agent/team scope 与生命周期已冻结 | 三类可验证 scope、候选生成、不可变修订链、原子发布/索引、失效/到期/tombstone、检索及 REST API 已实现；team 在 Goal G 前失败关闭 | 状态、chunk/embed、反思、验证、生命周期和 OpenAPI 规则已覆盖 | RLS、scope、HTTP 双租户、working TTL、修订/supersede、发布/检索及召回排除通过 | 成功 Run 候选、审批前隔离、发布后召回及第二租户隔离通过 | Goal F 计划、API、边界与 Handoff 已完成 | Goal F 已验证 |
...
| REST API 与 OpenAPI | 已设计 | 健康、控制面、Runtime/Tool 查询，以及 Goal F 候选、Memory、Skill/Version、Evaluation/Approval、deployment 全生命周期已实现 | 172 单测含路径/Header/成长响应敏感字段排除 | 58 项真实集成含完整成长 HTTP 与第二租户隔离 | Goal F 全部成长路径及 OpenAPI 敏感字段检查通过 | `docs/api.md` | Goal F 已验证；认证/SSE/SDK 待后续 |
...
| PostgreSQL/pgvector/Redis/MinIO | 已决策 | Runtime/Tool、成长表、vector(384)/HNSW、FORCE RLS、评价/审批唯一索引、不可变来源、原子 Memory 发布与 Skill release guards 已落库 | 探针/状态/chunk/embed/反思/验证/分桶已完成 | 58 项真实依赖/控制面/Runtime/Tool/Growth/vector/HTTP 测试通过 | API/Worker/Tool 与 Goal F 成长闭环使用真实 PostgreSQL/pgvector 通过 | ADR、API、领域、Tool、Memory/Skill 文档已更新 | Goal F 已验证 |
| Docker Compose 与一键脚本 | 已规划 | 9 服务拓扑含非 root Worker 状态卷初始化与独立 Runner；Goal F E2E/总验收脚本已实现 | Shell/Compose 静态校验通过 | 固定镜像、迁移、依赖和 Runner 健康已验证 | `verify-goal-f.sh` 与 Goal C/D/E/F E2E 全部通过 | 开发/测试/Handoff 已完成 | Goal F 已验证 |
| 全量单元、集成、故障与 E2E 测试 | 已规划 | Goal E 回归出口及 Goal F F2–F8 数据库/检索/候选/审核/Memory/Skill 服务、API、E2E 与证据已实现 | 后端 172 + 前端 7 通过 | 真实依赖 58 项通过 | Goal C 核心、Goal D Runtime、Goal E Tool/Sandbox、Goal F Growth E2E 全部通过 | `docs/testing.md` 与验收证据已完成 | Goal F 已验证 |
