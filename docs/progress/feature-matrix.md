...
| Candidate、验证、审批、发布与回滚 | content-hash 绑定的 Candidate→Evaluation→Approval→发布/灰度/回滚已冻结 | 反思、确定性 validator、不可变 Evaluation、非自审 Approval、Memory 发布与数据库双闸门已实现；Skill 发布/回滚待 F6 | 候选、Hash/Schema/工具/来源验证与 verdict 一致性已覆盖 | 并发幂等、最新评价、审批全终态、原子发布回滚、来源不可改写通过 | 未实现 | ADR-0005/0010 与阶段计划已完成 | F5 Memory 发布已验证；Skill 发布/回滚待 F6 |
...
| PostgreSQL/pgvector/Redis/MinIO | 已决策 | Runtime/Tool、成长表、vector(384)/HNSW、FORCE RLS、评价/审批唯一索引、不可变来源及原子发布事务已落库 | 探针/状态/chunk/embed/反思/验证已完成 | 53 项真实依赖/控制面/Runtime/Tool/Growth/vector 测试通过 | API、Worker 与 Tool Gateway 使用真实 PostgreSQL 已通过；Memory E2E 待 F8 | ADR、领域、Tool、Memory/Skill 文档已更新 | Goal F F5 Memory 生命周期已验证 |
...
| 全量单元、集成、故障与 E2E 测试 | 已规划 | Goal E 测试出口及 Goal F F2–F5 数据库/检索/候选/审核/Memory 生命周期测试已实现 | 后端 167 + 前端 7 通过 | 真实依赖 53 项通过 | Goal C 核心、Goal D Runtime、Goal E Tool/Sandbox E2E 通过；Goal F E2E 未实现 | `docs/testing.md` | Goal F F5 回归已通过 |
