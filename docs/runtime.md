...
- 执行：`hermes chat -q <envelope> -Q`，按配置映射 `--model`、`--provider`、`--toolsets` 和 `--resume`；
...
- Secret：只继承 Worker 环境，不进入 AgentVersion；stdout/stderr 错误脱敏后才可能写入结果；
- pause：Hermes 0.18.2 不声明运行中 pause；历史 session resume 用于租约恢复，不冒充 pause/resume 对称能力。

Goal D 尚未实现平台 Tool Gateway/Sandbox。Hermes 默认传入隔离占位 toolset `nico-disabled`；若开发者显式启用 Hermes 原生 toolset，那仍不是平台权限、ToolCall 审计或沙箱，不能用于生产安全声明。Goal E 必须把实际工具执行收口到平台边界。

当前主机存在 Hermes 0.18.2 源码参考，但没有安装 `hermes` 可执行文件，也没有外部模型验收凭据。因此 Goal D 只声称假 CLI 进程合同和本地源码兼容检查通过，不声称真实模型推理成功。
